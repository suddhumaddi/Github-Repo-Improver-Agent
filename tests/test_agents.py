...
from agents.content_improver import ContentSuggestions 

# --- Simulated Data for Testing Isolation ---

# Content used to simulate a README file for testing extraction logic
...
class TestAgentCoreLogic(unittest.TestCase):
    ...
    def test_repo_analyzer_process_success(self, MockShutil, MockRepo, MockLoader, mock_exists):
        ...
        # Configure the loader to return simulated documents
        mock_doc = MagicMock()
        mock_doc.load.return_value = [MagicMock(page_content=SIMULATED_FILE_CONTENT)]
        ...
        with patch.object(self.analyzer, '_clone_repo') as mock_clone, \
            ...
            # Process content using simulated environment
    ...
    def test_langgraph_flow_integrity(self, MockRepo, MockLoader, mock_exists):
        ...
        # 1. Setup simulated content
        mock_doc = MagicMock()
        mock_doc.load.return_value = [MagicMock(page_content=SIMULATED_FILE_CONTENT)]
        MockLoader.return_value = mock_doc
        
        # 2. Mock Agent 3 (ContentImprover) output to bypass external API/network calls
