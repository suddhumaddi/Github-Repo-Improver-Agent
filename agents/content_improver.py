...
class ContentImproverAgent:
    ...
    def __init__(self, retriever):
        # Targeting GPT-4o Mini for optimal JSON stability
        self.llm = ChatOpenAI(
            model="openai/gpt-4o-mini",  
            openai_api_key=os.getenv("OPENROUTER_API_KEY"),
            openai_api_base="https://openrouter.ai/api/v1",
            temperature=0.0 # Low temperature for reliable structured output
        )
    ...
    def generate_improved_content(self, original_content: str, metadata: dict):
        ...
        prompt_template = PromptTemplate(
            template="""You are an expert GitHub repository analyst. Your task is to analyze the content and suggest improvements. The output must strictly adhere to the provided JSON schema.
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                # LLM provided bad JSON. This is an agent failure, not a network failure. Fail immediately.
                ...
                # Handle API/Network issues (the target of resilience)
                ...
                # Exponential Backoff: Wait longer before the next attempt
